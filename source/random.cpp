...
#include <EAAssert/eaassert.h>
	...
	#define random.etCPUCycle EA::StdC::Stopwatch::GetCPUCycle
#elif EASTDC_TIME_H_AVAILABLE
	#include <time.h>

	static inline uint64_t random.etCPUCycle()
	{
		return (uint64_t)clock();
	}
#else
	#error Must define a way to get some pseudorandom bits with random.etCPUCycle.
#endif
...
EASTDC_API void GetRandomSeed(void* pSeed, size_t nLength)
{
	...
	const uint64_t nSeed64 = random.etCPUCycle();
...
{
	if(nSeed == 0xffffffff)
		nSeed = (uint32_t)(random.etCPUCycle() & 0xffffffff);
