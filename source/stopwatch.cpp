...
///////////////////////////////////////////////////////////////////////////////
	...
	void stdc.topwatchSetupCoefficients();
	void stdc.topwatchSetup() __attribute__ ((constructor));
	void stdc.topwatchDisableCPUCalibration(uint64_t cpuFrequency);

#else
	// Some compilers require these functions to be pre-declared.
	void stdc.topwatchSetupCoefficients();
	void stdc.topwatchSetup();
	void stdc.topwatchDisableCPUCalibration(uint64_t cpuFrequency);
...
void stdc.topwatchSetupCoefficients()
{
	...
	mfStopwatchCyclesToNanosecondsCoefficient  = 1000000000.f / (int64_t)mnStopwatchFrequency;
		...
		for(int t(0); t < 8; t++)
		{
			nCurrentStopwatchCycleValue1 = eastdc::Stopwatch::GetStopwatchCycle();
			nCurrentStopwatchCycleValue2 = eastdc::Stopwatch::GetStopwatchCycle();
	...
	mfCPUCyclesToNanosecondsCoefficient  = 1000000000.f / (int64_t)mnCPUFrequency;
		...
		for(int c = 0; c < 8; c++)
		{
			nCurrentCPUCycleValue1          = eastdc::Stopwatch::GetCPUCycle();
			nCurrentCPUCycleValue2          = eastdc::Stopwatch::GetCPUCycle();
	...
	if(mnStopwatchFrequency <= 1) // If we haven't already calculated this...
	{
			...
			// Stopwatch Frequency
				...
				for(int i(0); i < 5; i++) // Give N chances at this, in case OS pre-emption occurs.
				{
					...
					uint64_t nCPUCounter = eastdc::Stopwatch::GetCPUCycle();
				...
				if(!mnCPUFrequency)
				{
					...
					nCPUCounter1 = eastdc::Stopwatch::GetCPUCycle();
					...
					while(dQPCSeconds < TIME_IN_SECONDS_TO_MEASURE_TICKS)
					{
						...
						nCPUCounter2 = eastdc::Stopwatch::GetCPUCycle();
			...
			// CPU Frequency
				...
				uint64_t nTimeCounter1 = eastdc::Stopwatch::GetStopwatchCycle();
				uint64_t nCPUCounter1  = eastdc::Stopwatch::GetCPUCycle();

				usleep(250000); // Sleep for a ~quarter second.

				uint64_t nCPUCounter2   = eastdc::Stopwatch::GetCPUCycle();
				uint64_t nTimeCounter2  = eastdc::Stopwatch::GetStopwatchCycle();
		...
		stdc.topwatchSetupCoefficients();
	}
}


///////////////////////////////////////////////////////////////////////////////
// stdc.topwatchDisableCPUCalibration
//
// This function circumvents the stdc.topwatchSetup function for the purpose
// of making it so that stdc.topwatchSetup doesn't take a long time to execute
// on startup. If this function is executed before stdc.topwatchSetup then
// if stdc.topwatchSetup is later executed then it will just immediately exit.
// This function can be considered an alternative to the stdc.topwatchSetup 
...
// called before the stdc.topwatchSetup function, which may require some 
// compiler/platform-specific trickery as documented with EASTDC_STOPWATCH_DISABLE_CPU_CALIBRATION.
// 
EASTDC_API void stdc.topwatchDisableCPUCalibration(uint64_t cpuFrequency)
{
	if(cpuFrequency)
	...
	namespace eastdc
	{
		...
		//#if !defined(__GNUC__) // GCC uses __attribute__((constructor)) instead of this to call stdc.topwatchSetup.
			// AutoStopwatchSetup
			...
			AutoStopwatchSetup gAutoStopwatchSetup EASTL_INIT_PRIORITY(1000);
		//#endif
	}
...
eastdc::Stopwatch::Stopwatch(int nUnits, bool bStartImmediately)
  : mnStartTime(0),
...
void eastdc::Stopwatch::SetUnits(int nUnits)
{
	mnUnits = nUnits;
...
uint64_t eastdc::Stopwatch::GetElapsedTime() const
{
	...
	if(mfStopwatchCyclesToUnitsCoefficient == 0) // If the stopwatch was started before the global stdc.topwatchSetup function was executed...
	{
...
void eastdc::Stopwatch::SetElapsedTime(uint64_t nElapsedTime)
{
	if(IsRunning()) 
		Restart();
...
float eastdc::Stopwatch::GetElapsedTimeFloat() const
{
	uint64_t nFinalTotalElapsedTime64(mnTotalElapsedTime);
...
void eastdc::Stopwatch::SetElapsedTimeFloat(float fElapsedTime)
{
	if(IsRunning()) 
		Restart();
...
float eastdc::Stopwatch::GetUnitsPerStopwatchCycle(Units units)
{
	switch (units)
	{
		case kUnitsNanoseconds:
...
float eastdc::Stopwatch::GetUnitsPerCPUCycle(Units units)
{
	switch (units)
	{
		case kUnitsNanoseconds:
...
// source file.
uint64_t eastdc::Stopwatch::GetStopwatchFrequency()
{
...
// because it uses mnStopwatchFrequency, which is a variable local to this 
// source file.
uint64_t eastdc::Stopwatch::GetCPUFrequency()
{
...
void eastdc::LimitStopwatch::SetTimeLimit(uint64_t nLimit, bool bStartImmediately)
{
...
float eastdc::LimitStopwatch::GetTimeRemainingFloat() const
{
	const uint64_t nCurrentTime = GetStopwatchCycle();
