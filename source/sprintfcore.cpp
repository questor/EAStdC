		...
		if(nCount)
		{
			...
			for(size_t i = 0; i < nCount; i++)
			{
				...
				if((pData[i] == '\n') || (pWriteInfo->mPosition == kMaxCountActual)) // If the user is requesting a newline or if we have exhausted the most we can write in a single line...
				{
					...
					__android_log_write(ANDROID_LOG_INFO, "stdc.Printf", pWriteInfo->mBuffer);
...
CharT* WriteLongHelper(const SprintfLocal::FormatData& fd, ValueT lValue, CharT* EASTL_RESTRICT pBufferEnd)
{
	...
	*--pCurrent = 0;
	...
	if((lValue > 0) || (fd.mnPrecision > 0) || fd.mbAlternativeForm)
	{
		...
		// To do: Use the optimization in string.cpp's X64toaCommon10 function 
				...
				case 'i':    // The standard specifies that 'd' and 'i' are identical.
				{
					...
					else if(fd.mModifier == kModifierInt128)
					{
						...
						else
						{
							...
							// add the functionality later. We have the stdc.int128_t type we can use.
							// I don't think calling two 64 bit va_args is the same as what a single
							// 128 bit arg would be. If we are using stdc.int128_t then we handle the 
