...
EASTDC_API char* UTF8Increment(const char* p, size_t n)
{
	while(n--)
	{
		// To do: Change this code to instead use the utf8lengthTable fropm string.cpp
...
EASTDC_API bool WildcardMatch(const char* pString, const char* pPattern, bool bCaseSensitive)
{
	...
	else
	{
		...
		{
			pStringLAllocated = EASTDC_NEW("textutil.StringAllocated/char[]") char[nStringLLength + 1];
		...
		{
			pPatternLAllocated = EASTDC_NEW("textutil.PatternAllocated/char[]") char[nPatternLLength + 1];
...
EASTDC_API bool WildcardMatch(const char16_t* pString, const char16_t* pPattern, bool bCaseSensitive)
{
	...
	else
	{
		...
		if(nStringLLength >= (sizeof(pStringLBuffer) / sizeof(pStringLBuffer[0]) - 1))
		{
			pStringLAllocated = EASTDC_NEW("textutil.StringAllocated/char16[]") char16_t[nStringLLength + 1];
			pStringL          = pStringLAllocated;
		}
		else
		{
		...
		if(nPatternLLength >= (sizeof(pPatternLBuffer) / sizeof(pPatternLBuffer[0]) - 1))
		{
			pPatternLAllocated = EASTDC_NEW("textutil.PatternAllocated/char16[]") char16_t[nPatternLLength + 1];
			pPatternL          = pPatternLAllocated;
		}
		else
		{
...
EASTDC_API bool WildcardMatch(const char32_t* pString, const char32_t* pPattern, bool bCaseSensitive)
{
	...
	else
	{
		...
		if(nStringLLength >= (sizeof(pStringLBuffer) / sizeof(pStringLBuffer[0]) - 1))
		{
			pStringLAllocated = EASTDC_NEW("textutil.StringAllocated/char32[]") char32_t[nStringLLength + 1];
			pStringL          = pStringLAllocated;
		}
		else
		{
		...
		if(nPatternLLength >= (sizeof(pPatternLBuffer) / sizeof(pPatternLBuffer[0]) - 1))
		{
			pPatternLAllocated = EASTDC_NEW("textutil.PatternAllocated/char32[]") char32_t[nPatternLLength + 1];
			pPatternL          = pPatternLAllocated;
		}
		else
		{
