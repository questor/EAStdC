...
// clearer, singleton.is probably a better choice.
///////////////////////////////////////////////////////////////////////////////
...
namespace
{
	...
	{
		typedef eastdc::intrusive_list<eastdc::OSGlobalNode> OSGlobalList;
		...
		eastdc::OSGlobalNode* Find(uint32_t id);
		void                    Add(eastdc::OSGlobalNode* p);
		void                    Remove(eastdc::OSGlobalNode* p);
	};

	OSGlobalManager::OSGlobalManager() {
		eastdc::AtomicSet(&mRefCount, 0);
	}

	eastdc::OSGlobalNode* OSGlobalManager::Find(uint32_t id) {
		OSGlobalList::iterator it(mOSGlobalList.begin()), itEnd(mOSGlobalList.end());

		for(; it!=itEnd; ++it) {
			eastdc::OSGlobalNode& node = *it;
	...
	void OSGlobalManager::Add(eastdc::OSGlobalNode *p) {
		mOSGlobalList.pushFront(*p);
	}

	void OSGlobalManager::Remove(eastdc::OSGlobalNode *p) {
	...
	namespace {
		...
		bool InitOSGlobalSystem()
		{
			...
			if (!gpOSGlobalManager)
			{
				...
				// this process share this same underlying mutex. 
					...
					eastdc::Sprintf(uniqueName, EA_GLOBAL_UNIQUE_NAME_FORMAT_W, (unsigned)GetCurrentProcessId());
					HANDLE hMutex = CreateMutexExW(NULL, uniqueName, CREATE_MUTEX_INITIAL_OWNER, SYNCHRONIZE);
				#else
					// The kernel object namespace is global on Win32 so we have to choose a unique name.
					char uniqueName[64];

					eastdc::Sprintf(uniqueName, EA_GLOBAL_UNIQUE_NAME_FORMAT, (unsigned)GetCurrentProcessId());
				...
				if (WAIT_FAILED != WaitForSingleObjectEx(hMutex, INFINITE, FALSE))
				{
					// If we don't have access to GetEnvironmentVariable and are a 32 bit platform...
						...
						eastdc::Sprintf(uniqueSemaphoreName, L"SingleMgr%u", (unsigned)GetCurrentProcessId());
					...
					// If we don't have access to GetEnvironmentVariable and are a 64 bit platform...
						...
						eastdc::Sprintf(uniqueNameHi, L"SingleMgrHi%u", dwProcessId);
						eastdc::Sprintf(uniqueNameLo, L"SingleMgrLo%u", dwProcessId);
						...
						else
						{
							...
							eastdc::Sprintf(stringPtr, "%I64x", (uint64_t)(uintptr_t)gpOSGlobalManager);
					...
					eastdc::AtomicIncrement(&gpOSGlobalManager->mRefCount);
				...
				eastdc::AtomicIncrement(&gOSGlobalRefs);  // Increment it once for the init of this system (InitOSGlobalSystem). This increment will be matched by a decrement in ShutdownOSGlobalSystem.
			}
		...
		void ShutdownOSGlobalSystem()
		{
			if (eastdc::AtomicDecrement(&gOSGlobalRefs) == 0) // If the (atomic) integer decrement results in a refcount of zero...
			{
				if (gpOSGlobalManager)
				{
					if (eastdc::AtomicDecrement(&gpOSGlobalManager->mRefCount) == 0)
	...
	bool InitOSGlobalSystem()
	{
		...
		if(!gpOSGlobalManager)
		{
			...
			while (!gpOSGlobalManager && (currentAddr >= (kAddr64 - eastdc::kKettleOSGlobalSearchSpace)))
			{
				...
				if((result == SCE_KERNEL_ERROR_ENOMEM) || (result == SCE_OK))
				{
					...
					// done by a previous global.init occurred. With either that or SCE_OK, we call scekernelMapDirectMemory.
					...
					if((result == SCE_KERNEL_ERROR_ENOMEM) || (result == SCE_OK))
					{
						...
						if(weWereHereFirst)
						{
							...
							// reason global.exists is to allow sharing variables anonymously between modules. So we have a chicken and
							...
							eastdc::AtomicIncrement(&pOSGlobalManagerContainer->mOSGlobalManager.mRefCount);
						...
						else // Else somebody before us mapped this memory. We need to validate it before trying to use it.
						{
							...
							{
								eastdc::LimitStopwatch limitStopwatch(eastdc::Stopwatch::kUnitsMilliseconds, 500, true);
	...
	void ShutdownOSGlobalSystem()
	{
		...
		{ 
			if(gpOSGlobalManager)
			{
				if(eastdc::AtomicDecrement(&gpOSGlobalManager->mRefCount) == 0) // mRefCount measures the use count of glOSGlobalManager.
				{
	...
	namespace {
		...
		bool InitOSGlobalSystem()
		{
			...
			if(!gpOSGlobalManager)
			{
				...
				eastdc::Sprintf(uniqueName, EA_GLOBAL_UNIQUE_NAME_FORMAT, (unsigned long long)processID);
				...
				if(sem_wait(mutex) == 0) // If locking the mutex was successful...
				{
					...
					else
					{
						...
						{
							char buffer[32];
							eastdc::Sprintf(buffer, "%I64x", (uint64_t)(uintptr_t)gpOSGlobalManager);
	...
	namespace {
		...
		bool InitOSGlobalSystem()
		{
			...
			EA_ASSERT(gOSGlobalRefs < UINT32_MAX);
			if(eastdc::AtomicIncrement(&gOSGlobalRefs) == 1)
...
EASTDC_API eastdc::OSGlobalNode* eastdc::GetOSGlobal(uint32_t id, OSGlobalFactoryPtr pFactory)
{
	...
	eastdc::OSGlobalNode* p = gpOSGlobalManager->Find(id);
...
EASTDC_API bool eastdc::SetOSGlobal(uint32_t id, eastdc::OSGlobalNode *p)
{
	// Initialize up the OSGlobal system if we are getting called before
	// static init, i.e. allocator
	if (!InitOSGlobalSystem())
	...
	eastdc::OSGlobalNode* const pTemp = gpOSGlobalManager->Find(id);
...
EASTDC_API bool eastdc::ReleaseOSGlobal(eastdc::OSGlobalNode *p)
{
	gpOSGlobalManager->Lock();
