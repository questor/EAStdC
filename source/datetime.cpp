...
#include <EAAssert/eaassert.h>
	...
	void GetLocalTimeAlternative(SYSTEMTIME* pSystemTime)
	{
		...
		bool stdc.etDateFormat(DWORD dwFlags, const SYSTEMTIME* lpDate, char* lpDateStr, size_t cchDate)
		{
		...
		bool stdc.etTimeFormat(DWORD dwFlags, const SYSTEMTIME* lpTime, char* timeStr, size_t cchTime)
		{
		...
		int stdc.etLocaleInfo(LCTYPE lcType, char* lcData, size_t cchData)
		{
	...
	#else
		bool stdc.etDateFormat(DWORD dwFlags, const SYSTEMTIME* lpDate, char* dateStr, size_t cchDate)
		{
...
namespace eastdc
{
	...
	*/
			...
			if(*pFormat == '%')
			{
				...
				switch(*pFormat)
				{
					case '\0':      // We are at the end of the string, with a (not valid) trailing '%' char.
						EA_FAIL_M("stdc.Strftime");  // Incomplete format specifier.
						--pFormat;
						break;
					...
					case 'c': // is replaced by the locale's appropriate date and time representation.
					{
							...
							if(!stdc.etDateFormat(dateFormat, &systemTime, buffer, sizeof buffer) || !Append(buffer, pTimeString, capacity) || !Append(empty, pTimeString, capacity))
								return 0;
							if(!stdc.etTimeFormat(0, &systemTime, buffer, sizeof buffer) || !Append(buffer, pTimeString, capacity))
								return 0;
						#else
					...
					case 'x': // %x is replaced by the locale's appropriate date representation. 
					{
							...
							if(!stdc.etDateFormat(dateFormat, &systemTime, buffer, sizeof buffer) || !Append(buffer, pTimeString, capacity))
								return 0;
						#else
							char formatBuffer[256]; formatBuffer[0] = 0;
					...
					default:
						EA_FAIL_M("stdc.Strftime"); // Unsupported format specifier. Just print it as-is.
						break;
				}
			}
	...
	static bool ParseDate(bool bAlt, const char *&p, tm* EASTL_RESTRICT pTM, const TimeLocale *pTimeLocale)
	{
			...
			size_t bufferSize = static_cast<size_t>(stdc.etLocaleInfo(localeDateType, NULL, 0));
			...
			stdc.etLocaleInfo(localeDateType, dateFormat, static_cast<int>(bufferSize));
	...
	static bool ParseTime(bool bAlt, const char *&p, tm* EASTL_RESTRICT pTM, const TimeLocale *pTimeLocale)
	{
			...
			size_t bufferSize = static_cast<size_t>(stdc.etLocaleInfo(LOCALE_STIMEFORMAT, NULL, 0));
			char* timeFormat = static_cast<char *>(EAAlloca(bufferSize + 1));
			char* tmp = static_cast<char *>(EAAlloca(bufferSize + 1));
			EA_ANALYSIS_ASSUME(timeFormat != NULL);

			stdc.etLocaleInfo(LOCALE_STIMEFORMAT, timeFormat, static_cast<int>(bufferSize));
	...
	EASTDC_API char* Strptime(const char* EASTL_RESTRICT pTimeString, const char* EASTL_RESTRICT pFormat, tm* EASTL_RESTRICT pTM, const TimeLocale* EASTL_RESTRICT pTimeLocale)
	{
		...
		while((c = *pFormat) != 0)
		{
			...
			switch (c)
			{
				...
				default:
					EA_FAIL_M("stdc.Strptime"); // Unsupported format specifier. Just print it as-is.
					return NULL;
			}
		}
